...
{% for publi in site.data.publications.preprints %}
  **{{ publi.title }}** <br />
  ...
  {% if publi.link != null %} <a href="{{ publi.link }}">[ARXIV]</a> {% endif %}{% if publi.project != null %}<a href="{{ publi.project }}">[PROJECT]</a>{% endif %}{% if publi.endmsg != null %}  **{{ publi.endmsg }}** <br /> {% endif %}
